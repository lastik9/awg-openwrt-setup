# awg-openwrt-setup

**Русский** · [English](README.en.md) · [中文](README.zh-CN.md)

Установочный скрипт для **AmneziaWG** на **OpenWrt 25 (apk)** для совместной работы
с [podkop](https://podkop.net/) — выборочной маршрутизации трафика.

Один прогон: ставит пакеты AmneziaWG, создаёт сетевой интерфейс и firewall-зону с
NAT и MSS clamping, поднимает туннель и проверяет handshake. **Podkop не трогается** —
интерфейс подключается к нему вручную через LuCI. Есть отдельный аптинсталлер.

## Зачем

[AmneziaWG](https://docs.amnezia.org/) — это WireGuard с обфускацией, устойчивый к
блокировкам по DPI. Ручная установка на OpenWrt — это несколько шагов: пакеты,
интерфейс через веб-интерфейс, ручной импорт `.conf`, firewall-зона. При работе с
podkop добавляется нюанс: у интерфейса нельзя включать маршрутизацию всех
`AllowedIPs`, иначе весь трафик уйдёт в туннель мимо выборочных правил podkop.

Скрипт снимает эту рутину: собирает интерфейс и peer из простого `awg.env` (или
генерирует его из готового `.conf`), сразу ставит `route_allowed_ips=0` и создаёт
корректную firewall-зону.

## Требования

- OpenWrt 25.x с пакетным менеджером **apk** (проверялось на 25.12.x).
- Уже установленный и работающий **podkop**.
- Доступ по SSH и права **root**.
- Интернет на роутере на момент запуска — качаются пакеты AmneziaWG.

## Что делает скрипт

1. Ставит пакеты AmneziaWG через официальный установщик
   ([Slava-Shchipunov/awg-openwrt](https://github.com/Slava-Shchipunov/awg-openwrt)),
   неинтерактивно.
2. Читает параметры туннеля из `awg.env`.
3. Создаёт UCI-интерфейс и peer — **только если интерфейса ещё нет** (идемпотентно).
   Ставит `route_allowed_ips=0`, чтобы маршрутами управлял podkop.
4. Создаёт firewall-зону (`input`/`forward` REJECT, `output` ACCEPT, masquerade,
   MSS clamping). Пересылку `lan → awg` **не** добавляет — это делает podkop.
5. Поднимает интерфейс и проверяет наличие handshake.

Podkop скрипт **не настраивает** — после установки подключи интерфейс в
**Services → Podkop** вручную.

## Установка

Команды выполняются **на роутере** по SSH, в каталоге `/root`.

**Первая установка — одной командой** (скачать и сразу запустить мастер):

```sh
cd /root && wget -O setup-awg.sh "https://raw.githubusercontent.com/lastik9/awg-openwrt-setup/main/setup-awg.sh" && sh setup-awg.sh
```

> ⚠️ **Первая установка = один reboot.** Когда пакеты AmneziaWG ставятся на роутер
> впервые, `netifd` узнаёт про новый протокол только после перезагрузки. Скрипт сам
> это определит и в конце предложит перезагрузку с обратным отсчётом **15 секунд**:
> ничего не нажимаешь — роутер перезагрузится сам, после чего интерфейс поднимется
> автоматически; чтобы отказаться — введи `n` и нажми Enter (тогда выполнишь `reboot`
> вручную позже). На повторных запусках (протокол уже зарегистрирован) reboot не
> вызывается — туннель встаёт сразу.

Мастер проведёт по шагам:
- спросит **имя интерфейса** — придумай осмысленное, по стране/локации сервера
  (`awg_nl`, `awg_de`, `awg_us`), чтобы не путаться. Дефисы автоматически заменятся
  на подчёркивания (`awg-nl` → `awg_nl`);
- откроет **редактор** — вставь туда весь свой `.conf` (от `[Interface]` до конца
  `[Peer]`), сохрани и выйди (в `vi`: `Esc`, затем `:wq`);
- спросит **keepalive** — жми Enter для значения 25 (нужно за NAT).

Дальше скрипт сам поставит пакеты, создаст интерфейс с выбранным именем, добавит его
в общую firewall-зону `awg` и поднимет туннель.

**3. Подключи к podkop:** **LuCI → Services → Podkop**, тип подключения **VPN**,
сетевой интерфейс (то имя, что ты задал, напр. `awg_nl`) → Save & Apply.

### Альтернатива: без мастера

Если нужен неинтерактивный путь (автоматизация, скрипты) — сгенерируй `awg.env` из
`.conf` и запусти:

```sh
wget -O awg.env.example "https://raw.githubusercontent.com/lastik9/awg-openwrt-setup/main/awg.env.example"
sh setup-awg.sh --from-conf /root/my.conf          # создаст awg.env
sed -i "s/^KEEPALIVE=.*/KEEPALIVE='25'/" /root/awg.env
sh setup-awg.sh                                     # поднимет из awg.env
```

Либо заполни `awg.env` вручную из шаблона `awg.env.example`.

## Параметры запуска

| Флаг | Действие |
|------|----------|
| (без флагов) | мастер, если нет `awg.env`; иначе настройка из `awg.env` |
| `-i`, `--wizard` | принудительно запустить интерактивный мастер |
| `--from-conf FILE` | сгенерировать `awg.env` из `.conf` и выйти |
| `--no-install` | пропустить установку пакетов, только настройка |
| `--reboot` | тихий режим: перезагрузить роутер сразу, без 15-сек отсчёта (для автоматизации; = `AUTO_REBOOT=1`) |
| `--env PATH` | использовать env-файл по другому пути |
| `-h`, `--help` | справка |

Дополнительные опции задаются в `awg.env`:

| Переменная | Действие |
|------------|----------|
| `MAKE_ZONE='1'` | создать firewall-зону `awg` (0 — не трогать фаервол) |
| `KEEPALIVE='25'` | persistent keepalive; за NAT обязателен |

## Несколько серверов

Один интерфейс = один сервер. Чтобы добавить ещё один — просто **запусти мастер
снова** и дай интерфейсу другое имя:

```sh
sh setup-awg.sh -i
# имя: awg_de, вставь второй .conf — и всё
```

Каждый сервер получает свой интерфейс (`awg_nl`, `awg_de`, …), но все они попадают
в **одну общую firewall-зону** `awg` — поведение фаервола (masq + MSS) для всех
одинаковое, отдельные зоны плодить не нужно. Скрипт сам добавит новый интерфейс в
существующую зону.

В podkop каждый туннель подключается отдельной конфигурацией: добавь второй профиль
и укажи в нём нужный интерфейс. Какой трафик через какой сервер пускать — решаешь
списками в podkop.

## Важно про keepalive

В исходных `.conf` от Amnezia строки `PersistentKeepalive` обычно нет. Если роутер
стоит за NAT (в том числе за другим роутером), обязательно впиши `KEEPALIVE='25'` в
`awg.env` — иначе NAT-сессия будет закрываться и handshake отвалится. Интерфейс без
keepalive к тому же не начинает handshake сам, пока в туннель не пойдёт трафик.

## Удаление

```sh
wget https://raw.githubusercontent.com/lastik9/awg-openwrt-setup/main/uninstall-awg.sh
sh uninstall-awg.sh
```

Скрипт находит **все** интерфейсы с `proto=amneziawg` (мастер создаёт произвольные
имена в общей зоне), показывает список и по подтверждению сносит их вместе с
peer-секциями и firewall-зоной. Чтобы удалить только один интерфейс — укажи
`--iface awg_nl`. В конце скрипт **отдельно спрашивает**, сносить ли системные
пакеты AmneziaWG. Пакеты (`kmod-amneziawg`, `amneziawg-tools`,
`luci-proto-amneziawg`) — **общие**: если на роутере есть другие AWG-туннели, их
лучше оставить. Podkop скрипт не трогает.

> **Про podkop и DNS.** Если удаляемый интерфейс ещё подключён в podkop/forkop,
> singbox продолжит перехватывать DNS и слать его в мёртвый туннель: пинг по IP
> работает, а сайты по именам не открываются («интернет пропал»). Лучше снять
> интерфейс заранее в **Services → Podkop → Save & Apply**. Скрипт podkop **не
> редактирует**, но подстраховывает:
> - **до удаления** читает конфиг podkop/forkop и, если интерфейс там упомянут,
>   называет его поимённо и предупреждает;
> - **после удаления**, если ссылка осталась, предлагает остановить сервис
>   podkop/forkop и перезапустить firewall+dnsmasq, чтобы сразу вернуть интернет
>   (выборочная маршрутизация выключится, пока не поправишь podkop). Ребут это не
>   лечит — при автозапуске DNS отвалится снова, поэтому убери интерфейс из podkop.

Если после удаления интерфейсы «зависли» в LuCI с пометкой на удаление (kernel-девайс
держится, пока не перезагрузишь netifd — особенно если пакеты AWG уже сняты), скрипт
это обнаружит и предложит **reboot с 15-сек обратным отсчётом** (как при установке).
Пустой Enter/таймаут — перезагрузка; `n` + Enter — отмена. Флаг `--reboot` — тихая
перезагрузка без отсчёта (для автоматизации).

## Безопасность

- **Никогда** не коммить реальный `awg.env` — в нём приватный ключ. Его закрывает
  `.gitignore`.
- В репозиторий идёт только `awg.env.example` (шаблон без значений).
- Если ключи где-то засветились — перевыпусти конфиг и отзови старый на сервере.

## Благодарности

Проект — лишь установщик. Основная работа сделана в апстримах:

- [amnezia-vpn/amneziawg](https://docs.amnezia.org/) — протокол AmneziaWG;
- [Slava-Shchipunov/awg-openwrt](https://github.com/Slava-Shchipunov/awg-openwrt) —
  сборки пакетов AmneziaWG под OpenWrt;
- [podkop](https://podkop.net/) — выборочная маршрутизация.

Устанавливаемые компоненты принадлежат их авторам и распространяются под их
собственными лицензиями. Настоящая лицензия (MIT) покрывает только код этого скрипта.

## Отказ от ответственности

Скрипт предоставляется как есть. Перед первым запуском убедись, что у тебя есть
физический доступ к роутеру на случай отката. Проверяй сгенерированный `awg.env`
глазами перед применением.

## Лицензия

[MIT](LICENSE) © 2026 lastik9
