...
> это определит, предупредит и попросит выполнить `reboot` — после загрузки интерфейс
> поднимется автоматически. Чтобы скрипт перезагрузил роутер сам, добавь `--reboot`:
>
> ```sh
...
> ```
>
> На повторных запусках (протокол уже зарегистрирован) reboot не нужен — туннель встаёт сразу.
...
| `--reboot` | сам перезагрузить роутер, если для регистрации proto нужен reboot (= `AUTO_REBOOT=1`) |
