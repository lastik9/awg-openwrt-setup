...
> **Сначала отключи интерфейс в podkop.** Если удаляемый интерфейс ещё подключён
> в podkop/forkop, singbox продолжит перехватывать DNS и слать его в мёртвый
> туннель: пинг по IP работает, а сайты по именам не открываются («интернет
> пропал»). Перед удалением сними интерфейс в **Services → Podkop → Save & Apply**.
